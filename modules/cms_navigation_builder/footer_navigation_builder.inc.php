...
$pageName = 'Footer Navigation Builder';
...
switch($_GET['action']){
 ...
 case 'edit':
   ...
   echo '<form method="post" action="'.REGULAR_URL.'administration/index.php?group=3&module=3&component=2&action=edit_submit&buttonID='.$row['button_id'].'">';
 ...
 case 'edit_submit':
   ...
   $query .= 'button_class="0", ';
   $query .= 'http_type="D" ';
   $query .= 'WHERE button_id="'.$_GET['buttonID'].'"';
   $query .= ' AND site_id="'.$_SESSION['siteData']['site_id'].'"';
   $db->query( $query );
   echo '<script type="text/javascript" language="Javascript">';
   echo 'window.location = JSRL+"administration/index.php?group=3&module=3&component=2";';
   echo '</script>';
   break;
 case 'delete':
   $query = 'SELECT * FROM navigation WHERE button_id="'.$_GET['buttonID'].'" AND site_id="'.$_SESSION['siteData']['site_id'].'"';
 ...
 case 'add':
   $IsForm='true';
   echo '<form method="post" action="'.REGULAR_URL.'administration/index.php?group=3&module=3&component=2&action=add_submit">';
   echo '<table cellspacing="0" cellpadding="1" class="cms_tabular" style="padding:10px 0px 10px 10px;">';
   echo '<tr>';
   echo '<td style="width:100px;"><label for="display_text">Display Text: </label></td>';
 ...
 case 'add_submit':
   ...
   $query .= '"'.$_POST['display_text'].'","'.$_POST['link_format'].'","'.$_POST['page_id'].'","'.($row['num']+1).'","'.$_POST['http_type'].'","F", "'.$_POST['button_class'].'", "'.$_SESSION['siteData']['site_id'].'" )';
   $db->query( $query );
   echo '<script type="text/javascript" language="Javascript">';
   echo 'window.location= JSRL+"administration/index.php?group=3&module=3&component=2";';
   echo '</script>';
 ...
 case 'reorder':
   ...
   break;
 default:
   echo '<form method="post" action="'.REGULAR_URL.'administration/index.php?group=3&module=3&component=2&action=reorder&buttonID='.$row['button_id'].'">';
   ...
   $query = 'SELECT * FROM navigation WHERE navigation="F" AND site_id="'.$_SESSION['siteData']['site_id'].'" ORDER BY display_order ASC';
   $result = $db->query( $query );
   ...
   echo '<td style="border-right:1px solid #c6c6c6;"><center><a href="'.REGULAR_URL.'administration/index.php?group=3&module=3&component=2&action=edit&buttonID='.$row['button_id'].'" class="edit">&nbsp;</a></center></td>';
   echo '<td><center><p onclick=\'javascript: VerifyDelete("'.REGULAR_URL.'administration/index.php?group=3&module=3&component=2&action=delete&buttonID='.$row['button_id'].'");\' class="delete">&nbsp</p></center></td>';
   echo '</tr>';
   $i++;
   }
   echo '<tr>';
